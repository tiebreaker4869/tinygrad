...
def test_value_with_torch():
    ...
    c_val = a_val + b_val
    c_val.backward()
    assert torch.isclose(torch.tensor(c_val.data), c)
    assert torch.isclose(torch.tensor(a_val.grad), a.grad)
    assert torch.isclose(torch.tensor(b_val.grad), b.grad)
    ...
    b_val = a_val ** 3
    b_val.backward()
    assert torch.isclose(torch.tensor(b_val.data), b)
    assert torch.isclose(torch.tensor(a_val.grad), a.grad)
...
def test_value_with_torch_complex():
    ...
    d_val.backward()
    assert torch.isclose(torch.tensor(d_val.data), d)
    assert torch.isclose(torch.tensor(a_val.grad), a.grad)
    assert torch.isclose(torch.tensor(b_val.grad), b.grad)
    assert torch.isclose(torch.tensor(c_val.grad), c.grad)
    ...
    e_val.backward()
    assert torch.isclose(torch.tensor(e_val.data), e)
    assert torch.isclose(torch.tensor(a_val.grad), a.grad)
    assert torch.isclose(torch.tensor(b_val.grad), b.grad)
    assert torch.isclose(torch.tensor(c_val.grad), c.grad)
    assert torch.isclose(torch.tensor(d_val.grad), d.grad)
    ...
    # Complex operation 4: (a ** 2 + b ** 3) / (c - d)
    a = torch.tensor(2.0, requires_grad=True)
    b = torch.tensor(3.0, requires_grad=True)
    c = torch.tensor(4.0, requires_grad=True)
    d = torch.tensor(5.0, requires_grad=True)
    ...
    c_val = Value(4.0)
    d_val = Value(5.0)
    e_val = (a_val ** 2 + b_val ** 3) / (c_val - d_val)
    e_val.backward()
    assert torch.isclose(torch.tensor(e_val.data), e)
    assert torch.isclose(torch.tensor(a_val.grad), a.grad)
    assert torch.isclose(torch.tensor(b_val.grad), b.grad)
